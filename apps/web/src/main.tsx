...
function App() {
  ...
  const visibleSpots = useMemo(
    () => spots
      .filter((spot) => spot.kind === mode)
      .sort((left, right) => (right.likes ?? 0) - (left.likes ?? 0) || left.name.localeCompare(right.name, "ko")),
    [mode, spots],
  );
