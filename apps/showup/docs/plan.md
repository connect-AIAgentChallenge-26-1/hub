...
- 진행 방식: **Conductor 4세션** (프론트 / 백엔드 / 시큐리티 / 리드)
...
2. 성실한 방문 이력이 쌓이면 "신뢰 배지"를 받는다
3. 새 가게 예약 시 배지를 제시해 예약금 면제 등 혜택을 받는다
...
나머지 전부 확장: 통계 차트(Phase 1.5), 고객 본인 조회·신뢰 배지·예약금 결제(Phase 2), 외부 예약 플랫폼 연동(Phase 3)
...
Phase 1.5(통계 차트)·Phase 2(본인 조회/배지/예약금)·Phase 3(플랫폼 연동)는 7/30 이후.

---

## 10. Conductor 세션 구조 (4 lanes)
...
- Conductor 세션은 **git 브랜치 분리가 아니라 역할 분리** — 같은 브랜치에서 영역별 작업
- commit / push / PR은 **리드 세션만** 수행
