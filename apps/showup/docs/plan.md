...
> - 수요일 = 현업 특강 예정 (시간 미정)
>
> 1일차(7/9 목) 완료분은 이미 커밋됨.
> 개발 가능일 = 13일 (금요일 3일 제외), 남은 개발일 = 12일.

| 일차 | 날짜 | 요일 | 개발 | 단계 |
|------|------|------|------|------|
| 1일차 | 7/09 | 목 | ✅ | **기반 세팅** (완료) |
| 2일차 | 7/10 | 금 | ❌ | 발표·데모·피드백 (1일차 성과) |
| 3일차 | 7/13 | 월 | ✅ | 인증·폼 (RHF+Zod, Firebase Auth) |
| 4일차 | 7/14 | 화 | ✅ | 고객 CRUD·목록·검색 |
| 5일차 | 7/15 | 수 | ✅ | 예약 생성·이벤트 기록 (노쇼·사건) |
| 6일차 | 7/16 | 목 | ✅ | 위험도 계산·riskStats 연동 |
| 7일차 | 7/17 | 금 | ❌ | 발표·데모·피드백 (3~6일차 성과) |
| 8일차 | 7/20 | 월 | ✅ | 위험도 UI·경고 배너·대시보드 |
| 9일차 | 7/21 | 화 | ✅ | 대시보드 완성·상태 UI·고객 상세 |
| 10일차 | 7/22 | 수 | ✅ | 모바일 QA·성능·보안 강화 |
| 11일차 | 7/23 | 목 | ✅ | 침투 테스트·규칙 최종·통합 QA |
| 12일차 | 7/24 | 금 | ❌ | 발표·데모·피드백 (8~11일차 성과) |
| 13일차 | 7/27 | 월 | ✅ | 버그 수정·데모 데이터·배포 |
| 14일차 | 7/28 | 화 | ✅ | 배포·스테이징·발표 초안 |
| 15일차 | 7/29 | 수 | ✅ | 발표 준비·최종 점검 |
| 16일차 | 7/30 | 목 | ✅ | **최종 마무리** |

**개발일 요약**: 13일 (1일차 완료 + 12일 남음)
**PR 제출 가능일**: 3일차(월), 4일차(화), 5일차(수), 6일차(목), 8일차(월), 9일차(화), 10일차(수), 11일차(목), 13일차(월), 14일차(화), 15일차(수), 16일차(목)

Phase 1.5(통계 차트)·Phase 2(본인 조회/정정·삭제 요청/이의제기)·Phase 3(플랫폼 연동)는 7/30 이후.

---

## 10. Hermes Agent 프레임워크 + Ollama Pro 모델 세션 구조 (4 lanes)

> **이관 배경**: Claude 구독 만료 + Codex 정지 이슈로 Ollama Pro 모델들을 Hermes Agent 프레임워크에 연결하여 기존 Conductor 기반 세션 구조를 이관. 같은 default 프로필에서 채팅 세션 4개를 열어 역할별로 운영.
...
- Hermes Agent 프레임워크 + Ollama Pro 모델 세션은 **git 브랜치 분리가 아니라 역할 분리** — 같은 브랜치에서 영역별 작업
- **커밋은 각 세션에서 하나의 작업이 끝날 때마다 자동 수행** — push와 PR은 사용자가 명시적으로 지시할 때만 실행
- **커밋 메시지 규칙**: 세션별 접두어 — `FE-` / `BE-` / `SEC-` / `LEAD-`
- 인터페이스 우선: 백엔드가 `types/schema.ts` + 시드 데이터 먼저 확정 → 프론트엔드는 mock으로 병행 개발
- 보안은 개발일(3/6/8/11일차)에 침투 테스트 수행, 금요일 발표 시 리포트 제출
- 세션 간 의존 충돌 시 리드가 결정
