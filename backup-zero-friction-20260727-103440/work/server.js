...
const COACH_SYSTEM_PROMPT = [
  ...
  'Markdown, 코드 펜스, 설명문을 JSON 밖에 절대 추가하지 마라.'
].join('\n');
  ...
  properties: {
    ...
    tasks: {
      ...
      items: {
        ...
        required: ['text', 'details', 'urgency', 'durationMinutes']
      }
    }
  },
  ...
  return {
    ...
    taskUpdates: output.taskUpdates.map((update, index) => {
      ...
      return {
        ...
        tasks: item.tasks.map((task, taskIndex) => requireTask(task, `taskUpdates[${index}].tasks[${taskIndex}]`))
      };
    }),
...
function createAuthorizationUrl(providerConfig, redirectUri, state) {
...
function oauthPopupResultHtml({ success, provider, message }) {
  const safePayload = JSON.stringify({
    type: success ? 'planner-auth-success' : 'planner-auth-error',
    provider,
    message
  }).replace(/</g, '\\u003c');
...
function createStore(dbPath) {
  ...
  const selectOAuthState = db.prepare(`
    SELECT provider, redirect_uri, expires_at
    FROM oauth_states
    WHERE state_hash = ?
  `);
  ...
  function createOAuthState(provider, redirectUri) {
    const state = crypto.randomBytes(32).toString('base64url');
    const now = new Date();
    const expiresAt = new Date(now.getTime() + OAUTH_STATE_TTL_MS);
    deleteExpiredOAuthStates.run(now.toISOString());
    insertOAuthState.run(hashAuthToken(state), provider, redirectUri, expiresAt.toISOString(), now.toISOString());
    return state;
  }
...
async function requestCoach({ fetchImpl, openAiApiKey, model, sessionId, payload, history }) {
  ...
  const recentConversation = payload.requestContext?.hasAttachment
    ? []
  ...
  return validateCoachOutput(parsedOutput);
}
...
async function requestUploadParse({ fetchImpl, openAiApiKey, model, sessionId, file, fields }) {
  ...
  const instruction = [
    ...
    '중복을 제거하고 각 작업을 5~240분 단위로 추정하세요.',
    `사용자 지시: ${sanitizeSensitiveString(fields.message || '파일의 핵심 일정을 오늘 계획으로 정리')}`,
    `사용자 모드: ${fields.mode || 'student'}, 현재 요일: ${fields.currentDay || 'mon'}`
  ].join('\n');
...
function createApp(overrides = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/api/briefing/personalized') {
    ...
    try {
      if (req.method === 'GET' && pathname === '/api/health') {
        return sendJson(res, 200, {
          ...
          model: config.model,
          capabilities: ['text', 'image', 'document', 'voice', 'personalized-briefing']
        });
      }
      ...
      if (req.method === 'GET' && authCallbackMatch) {
        ...
        if (oauthError) {
          return sendHtml(res, 400, oauthPopupResultHtml({
            success: false,
            provider,
            message: '로그인이 취소되었거나 계정 권한을 확인할 수 없습니다.'
          }));
        }
        ...
        if (!savedState || !code) {
          return sendHtml(res, 400, oauthPopupResultHtml({
            success: false,
            provider,
            message: '로그인 요청이 만료되었습니다. 플래너에서 다시 시도해 주세요.'
          }));
        }
