...
-- Zero-Friction Autonomous Co-pilot: 서버 전용 장기기억·상태·수습 이력
create extension if not exists vector with schema extensions;

create table if not exists public.autonomous_planner_states (
  owner_id text primary key,
  state_json jsonb not null default '{}'::jsonb,
  updated_at timestamptz not null default now()
);

create table if not exists public.planner_memories (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists planner_memories_owner_recent_idx
  on public.planner_memories (owner_id, last_accessed_at desc);
create index if not exists planner_memories_embedding_hnsw_idx
  on public.planner_memories using hnsw (embedding extensions.vector_cosine_ops)
  where embedding is not null;

create table if not exists public.autonomous_housekeeping_events (
  id uuid primary key default gen_random_uuid(),
  owner_id text not null,
...
create index if not exists autonomous_housekeeping_owner_recent_idx
  on public.autonomous_housekeeping_events (owner_id, created_at desc);

create table if not exists public.autonomous_push_tokens (
...
create or replace function public.match_planner_memories(
  ...
  match_threshold double precision default 0.54,
...
as $$
  ...
  from public.planner_memories as memory
  where memory.owner_id = query_owner_id
...
revoke all on function public.match_planner_memories(text, extensions.vector, double precision, integer) from public, anon, authenticated;
grant execute on function public.match_planner_memories(text, extensions.vector, double precision, integer) to service_role;

comment on table public.planner_memories is
