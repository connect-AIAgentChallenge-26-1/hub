...
현재 HEAD의 실행 코드는 Vite + React 소개 페이지, T00 계약 contract test, T01 backend 인증 기반(FastAPI+PostgreSQL), T02 S1·S2(종목 해석·OpenDART 공시 수집), T03 S13·S14(시세·기업행위·외부 근거 수집, 일부), T04 S3·S15(재무 계산·시점 정합성), T05 I9 골든 평가 하네스(대부분 완료, BLOCKED 1항목), T06 S7·S16·S17·S23(Structured Claim·결정론 검산, Solar 라이브 호출만 미검증), T07 S18·S19·S20·S8·S9(RAG·반증·인용·오케스트레이션·체크리스트)와 기능 C 결과 UI(S11 일부), T08 S1 확장(기업개황)·S4·S11·S20(기능 A 종목 공부 리포트), T09 S5·S6·S21(기능 B 가치 범위·가격 위치)다. S10·S12·S22(나머지 도메인 스킬)와 S11의 LLM 요약 경로(기능 A/B/C 공통)는 아직 구현되지 않았다. 문서가 목표 상태를 정의하더라도 실제 완료 여부는 [docs/checklist.md](docs/checklist.md)와 자동 테스트 결과로만 판단한다.
...
- [backend/](backend/) — FastAPI + Pydantic v2 + SQLAlchemy + Alembic + PostgreSQL.
  ...
  - T08(완료): S1 확장 기업개황(`app/providers/opendart.py` `fetch_company_overview` — OpenDART `company.json`, 2026-07-15 실 API 라이브 호출로 확인, `app/services/company_overview.py` `CompanyOverviewCollector`, `RawCompanyOverviewRecord` immutable). S4 전문용어 설명(`app/services/term_explainer.py` — 승인된 사전 20개 우선, 미등재 용어는 S23 게이트를 거친 주입 가능 콜백, 반환 전 금지 문구 재검사). S11 종목 공부 리포트(`app/services/company_report_generator.py` `CompanyReportGenerator` — 파이프라인 A 오케스트레이션, 기완성 S2·S3·S20(T07)을 재사용). `POST /api/v1/company-report`(`app/routers/company_report.py`), 결과 UI `src/components/CompanyReport.jsx`(기업개요·정정 배지·재무지표 추세·용어 glossary·확인 포인트·원문 인용 viewer). **부수 발견·수정**: 실제 삼성전자 현금흐름표 데이터로 `FinancialFactRow`/`FinancialFact`의 unique 제약이 DART placeholder 계정(표준계정코드 미사용)을 구분 못 하는 결함을 발견해 `ord` 필드를 두 제약에 추가(T04 `account_detail` 선례와 동일 원칙, `docs/skills.md` migration 기록), `FinancialCalculator.normalize()`의 비idempotent 저장 패턴도 발견해 `company_report_generator.py`에 국소 방어 추가(`docs/checklist.md` C5·`docs/backlog.md` T08 참고)
  - T09(완료): S21 비교군 구성(`app/services/peer_universe.py` `build_peer_universe` — 업종 일치(OpenDART 기업개황 `induty_code` KSIC 대분류)·자기 제외·기준일 일치·PER/PBR 유효성 4가지 포함·제외 규칙, P25/중앙값/P75 통계, `MIN_PEER_SAMPLE_SIZE=3` 미만이면 `sufficient=False`). S5 가치 시나리오(`app/services/valuation_scenarios.py` `compute_valuation_scenarios` — PER 상대가치·PBR 상대가치·ROE 조정 PBR 상대가치 3가지, peer P25/중앙값/P75 기반 저·중·고 3점 range, 단일 값 금지). S6 가격 위치(`app/services/price_position.py` `compute_price_position` — 방법별+종합 `BELOW/WITHIN/ABOVE_MODEL_RANGE/INSUFFICIENT`, 방법 간 불일치 노출). 오케스트레이터(`app/services/valuation_report_generator.py` `ValuationReportGenerator` — 파이프라인 B, 기완성 S1 확장(T08)·S2·S3·S13 재사용). `POST /api/v1/valuation-report`(`app/routers/valuation_report.py`), 결과 UI `src/components/ValuationReport.jsx`. 삼성전자(target)+SK하이닉스·DB하이텍·삼성전기(peer, 동일 KSIC 대분류 "26") 2026-07-15 라이브 캡처 데이터로 검증. **부수 발견·수정**: 실제 SK하이닉스·삼성전기 데이터로 `select_canonical_facts()`의 `METRIC_CANONICAL_SJ_DIV`가 IS 구획 없는 기업(CIS에만 손익계산서)에서 NET_INCOME 등을 경고 없이 `None`으로 드롭하는 결함을 발견해 IS→CIS fallback 추가(`docs/checklist.md` C6·`docs/backlog.md` T09 참고)
