...
1. **Task 확정** — [backlog.md](backlog.md)에서 지시받은 Task의 선행조건이 `완료`인지 확인한다. 미충족이면 구현하지 말고 `BLOCKED` 사유와 해제 조건을 보고한다.
...
6. [report/claude.md](report/claude.md)에 작업 보고를 남긴다. 파일을 읽지 말고 `cat >> docs/report/claude.md`로 하단에만 추가하며, 형식은 파일 상단 템플릿을 따른다.
