...
export function PlaceDetail({ jobId, placeId }: { jobId: string; placeId: string }) {
  ...
  return <article className="space-y-5">
    <Link href={`/recommendations/${jobId}`} className="text-sm font-bold text-teal-800">← 추천 결과로 돌아가기</Link>
    <section className="surface-card overflow-hidden">
      ...
      <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-2">
        <div><h2 className="text-lg font-black">근거 기반 점수 구성</h2><p className="mt-1 text-sm leading-6 text-slate-600">가격·인원은 검증 가능한 근거가 없어 점수에 포함하지 않습니다.</p><div className="mt-4"><ScoreDetails place={place} /></div></div>
        <div><h2 className="text-lg font-black">검증된 추천 이유</h2><p className="mt-1 text-xs font-semibold text-teal-800">{place.reasonSource === "GENERATED" ? "검증된 AI 생성 문장" : "서버 검증 템플릿 문장"}</p><ul className="mt-4 space-y-3">{place.reasonStatements.map((reason) => <li key={reason.text} className="rounded-xl bg-slate-50 p-4 text-sm leading-6">{reason.text}<span className="mt-1 block text-xs text-slate-400">근거 {reason.evidenceIds.join(", ")}</span></li>)}</ul></div>
      </div>
    </section>
  </article>;
}
