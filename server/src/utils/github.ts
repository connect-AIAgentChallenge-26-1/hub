...
// Resolves the connected repo + branch (project.json) and the logged-in
// user's OAuth token (session.json) into what every Contents API call needs.
// Returns null if either piece is missing, which callers surface as 401.
export async function getRepoTarget(): Promise<RepoTarget | null> {
  const session = await getSession();
  if (!session) return null;

  let project: StoredProject;
  try {
    project = await readJson<StoredProject>(dataPath("project.json"));
  } catch {
    return null;
  }
  ...
  return { owner, repo, branch: project.branch, accessToken: session.access_token };
}
